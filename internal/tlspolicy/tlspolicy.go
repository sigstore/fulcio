...
// disk needs a restart. nextProtos sets the ALPN list (use ALPNProtocols for
// the HTTP and duplex listeners; nil for gRPC, which handles HTTP/2 itself).
func (p Policy) ServerConfig(cert tls.Certificate, nextProtos []string) *tls.Config {
	/* #nosec G402 */ // MinVersion is a variable, but ParseVersion constrains it to TLS 1.2 or 1.3 and Resolve defaults it to DefaultMinVersion (TLS 1.3), so it is never below the G402 floor.
	return &tls.Config{
