...
func (fc *legacyIssuer) Match(ctx context.Context, url string) bool {
	// Catches all issuer urls so that it can be used as a fall back
	// implementation of an issuer.
	return true
}

func (fc *legacyIssuer) Authenticate(ctx context.Context, token string) (identity.Principal, error) {
	issuerURL, err := extractIssuer(token)
	if err != nil {
		return nil, err
	}

	verifier, ok := fc.getVerifier(issuerURL)
	if !ok {
		return nil, fmt.Errorf("unsupported issuer: %s", issuerURL)
	}
...
func (fc *legacyIssuer) getIssuer(issuerURL string) (OIDCIssuer, bool) {
...
// coming from an incoming OIDC token.  If no matching configuration
// is found, then it returns `false`.
func (fc *legacyIssuer) getVerifier(issuerURL string) (*oidc.IDTokenVerifier, bool) {
...
func (fc *legacyIssuer) prepare() error {
...
func parseConfig(b []byte) (cfg *legacyIssuer, err error) {
	cfg = &legacyIssuer{}
...
func validateConfig(conf *legacyIssuer) error {
	if conf == nil {
...
var defaultConfig = &legacyIssuer{
...
func with(ctx context.Context, cfg *legacyIssuer) context.Context {
	ctx = context.WithValue(ctx, configKey{}, cfg)
	return ctx
}

func fromContext(ctx context.Context) *legacyIssuer {
	untyped := ctx.Value(configKey{})
	if untyped == nil {
		return nil
	}
	return untyped.(*legacyIssuer)
}

// NewIssuerFromConfig loads a legacy issuer from disk, or use defaults
func NewIssuerFromConfig(configPath string) (identity.Issuer, error) {
	if _, err := os.Stat(configPath); os.IsNotExist(err) {
		log.Logger.Infof("No config at %s, using defaults: %v", configPath, defaultConfig)
		return NewDefaultIssuer()
	}
	b, err := ioutil.ReadFile(configPath)
	if err != nil {
		return nil, fmt.Errorf("read file: %w", err)
	}
	return NewIssuerFromBytes(b)
}

func NewDefaultIssuer() (identity.Issuer, error) {
