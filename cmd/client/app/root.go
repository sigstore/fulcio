...
var rootCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		config := oauth2.Config{
			ClientID: "22738965389-dcgdn3i91jjt1pbp1qdhg4e5c615sfjs.apps.googleusercontent.com",
			// THIS IS NOT A SECRET - IT IS USED IN THE NATIVE/DESKTOP FLOW.
			ClientSecret: "evnchIxIah0dixmsYcW-1EB1",
			Endpoint:     provider.Endpoint(),
			RedirectURL:  "http://127.0.0.1:5556/auth/google/callback",
			Scopes:       []string{oidc.ScopeOpenID, "email"},
		}

		oauthClient, err := oauth2ns.AuthenticateUser(&config)
		if err != nil {
			return err
		}
		...
		apiKeyQueryAuth := httptransport.APIKeyAuth("access_token", "query", oauthClient.Token.AccessToken)
