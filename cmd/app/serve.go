...
func StartDuplexServer(ctx context.Context, cfg *config.FulcioConfig, ctClient *ctclient.LogClient, baseca certauth.CertificateAuthority, algorithmRegistry *signature.AlgorithmRegistryConfig, host string, port, metricsPort int, ip identity.IssuerPool) error {
	...
	// Load the shared listener's TLS cert once at startup (rotating it needs a
	// restart). The duplex gateway dials itself over the loopback to turn REST
	// into gRPC, so when the listener speaks TLS that in-process dial has to use
	// TLS too, or the REST endpoint can't reach its own gRPC backend.
	var (
	...
	if tlsEnabled {
		lis = tls.NewListener(lis, policy.ServerConfig(cert, tlspolicy.ALPNProtocols))
