...
func StartDuplexServer(ctx context.Context, cfg *config.FulcioConfig, ctClient *ctclient.LogClient, baseca certauth.CertificateAuthority, algorithmRegistry *signature.AlgorithmRegistryConfig, host string, port, metricsPort int, ip identity.IssuerPool) error {
	...
	// Serve /metrics ourselves, like two-listener mode. go-grpc-kit's
	// RegisterListenAndServeMetrics re-registers the gRPC collectors on the
	// default registry, colliding with grpc_prometheus and panicking at startup.
	// TODO(https://github.com/sigstore/fulcio/issues/2419): drop the deprecated
	// go-grpc-prometheus dependency in favor of go-grpc-middleware/providers/prometheus.
	...
	defer func() {
		if err := metricsServer.Shutdown(context.Background()); err != nil {
			log.Logger.Errorf("metrics server shutdown: %v", err)
		}
	}()
